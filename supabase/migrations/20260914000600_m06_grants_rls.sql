-- M06: RLS y grants explicitos de las estructuras canonicas.
-- No habilitar RLS en tablas legacy con consumidores no relevados en este baseline.
...
grant select on public.perfiles, public.clientes, public.productos, public.planes,
  public.precios, public.ventas, public.venta_items, public.pagos, public.licencias,
  public.comisiones to authenticated;
grant insert, update, delete on public.perfiles, public.clientes, public.productos,
  public.planes, public.precios, public.ventas, public.venta_items, public.pagos,
  public.licencias, public.comisiones to authenticated;
...
create policy pagos_admin_total on public.pagos for all to authenticated
  using ((select app_private.es_admin())) with check ((select app_private.es_admin()));
create policy pagos_select_vendedor_propietario on public.pagos for select to authenticated
  using (venta_id is not null and (select app_private.es_vendedor_de_venta(venta_id)));

create policy licencias_admin_total on public.licencias for all to authenticated
  using ((select app_private.es_admin())) with check ((select app_private.es_admin()));
create policy licencias_select_vendedor_propietario on public.licencias for select to authenticated
  using (venta_id is not null and (select app_private.es_vendedor_de_venta(venta_id)));

create policy comisiones_admin_total on public.comisiones for all to authenticated
  using ((select app_private.es_admin())) with check ((select app_private.es_admin()));
