...
create table public.venta_items (
  ...
  created_at timestamptz not null default now(),
  check (
    (producto_id is null and plan_id is null and precio_id is null)
    or (producto_id is not null and plan_id is not null and precio_id is not null)
  )
);
...
comment on table public.venta_items is
  'Snapshot historico de producto, plan, cantidad y precio aplicados; no depende del catalogo actual.';
